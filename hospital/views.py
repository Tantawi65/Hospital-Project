...
from .forms import NurseUserForm, NurseForm
...
def nurse_signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if userForm.is_valid() and nurseForm.is_valid():
            ...
            # Add user to the "NURSE" group
            my_nurse_group, _ = Group.objects.get_or_create(name='NURSE')
            my_nurse_group.user_set.add(user)

            return HttpResponseRedirect('nurselogin')  # Redirect to nurse login
...
def nurse_login_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None and hasattr(user, 'nurse'):  # Ensure the user is a nurse
                login(request, user)
                return redirect("nurse_dashboard")  # Redirect to the nurse's dashboard (create this later)
            else:
    ...
    return render(request, "hospital/nurse_login.html", {"form": form})
